...
public:
	...
	vector<Vec3> move(Vec3 dst,Vec3 src){
		if(test_near_distance(dst-src)) return {src};
	...
	Vec3 getRB(const Vec3 p){
		auto it=g.getLeaves().lower_bound(make_pair(p,p));
		if(it!=g.getLeaves().end() && it->X==p) return it->Y;
		return Vec3{-1,-1,-1};
	}
	...
	vector<Vec3> getBox(Vec3 pos){
		if(g.getLeaves().begin()->X.y<0) return {};
...
int main(int argc, char* argv[]){
	...
	MyState s(v, 40);
	for(auto p:s.g.getRoots()) cout<<p<<",";cout<<endl;
