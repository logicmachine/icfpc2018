...
inline bool dispatch_move_no_conflict(State& s, int index, const Vec3& target){
	...
	if(d.x < -MAX_SHORT_DISTANCE){ s.bots(index).smove(Vec3(max(d.x, -MAX_LONG_DISTANCE), 0, 0)); return false; }
	if(d.x >  MAX_SHORT_DISTANCE){ s.bots(index).smove(Vec3(min(d.x,  MAX_LONG_DISTANCE), 0, 0)); return false; }
	if(d.y < -MAX_SHORT_DISTANCE){ s.bots(index).smove(Vec3(0, max(d.y, -MAX_LONG_DISTANCE), 0)); return false; }
	if(d.y >  MAX_SHORT_DISTANCE){ s.bots(index).smove(Vec3(0, min(d.y,  MAX_LONG_DISTANCE), 0)); return false; }
	if(d.z < -MAX_SHORT_DISTANCE){ s.bots(index).smove(Vec3(0, 0, max(d.z, -MAX_LONG_DISTANCE))); return false; }
	if(d.z >  MAX_SHORT_DISTANCE){ s.bots(index).smove(Vec3(0, 0, min(d.z,  MAX_LONG_DISTANCE))); return false; }
