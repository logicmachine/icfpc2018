...
//   return ret;
// }
bool isNextToLeaf(const set<Vec3> &leaf, Vec3 pos, int R){
  for(int dz=-1; dz<=1; dz++){
    for(int dy=-1; dy<=1; dy++){
      for(int dx=-1; dx<=1; dx++){
        ...
        if(leaf.count(Vec3(ddx, ddy, ddz))) return true;
      }
    }
  }
  return false;
    ...
    for(int d=0; d<6; d++){
      ...
      for(int i=1;i<=15;i++){
        ...
        if(vst.emplace(nxt).second){
          ...
          prv[nxt] = p;
          que.push(nxt);
          bool res = isNextToLeaf(leaf, nxt, s.matrix().r());
          if(res){
            vector<Vec3> root;
      ...
      for(int dy=-1; dy<=1; dy++){
        for(int dx=-1; dx<=1; dx++){
          ...
          if(graph.getLeaves().count(Vec3(ddx, ddy, ddz))){
            ...
            s.commit();
            graph.eraseLeaf(Vec3(ddx, ddy, ddz));
            f=true;
          }
        }
      }
